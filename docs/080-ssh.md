...
On Windows or iOS, search 'Terminal' in the launcher. You should see a small black box (called "Command Prompt" on Windows) with a cursor.
...
The terminal will look similar, but commands you run will now execute on this remote machine. You may also have also changed operating system, so you rather than the commands for iOS or Windows, you need to use the commands the work on the remote machine. Many HPC machines and webservers are linux machines (and so is the one we will connect to in class).
...
**These exercises will need to be done in class or immediately after since the remote machine is not turned on permamently**

* Find and run an mp3 file on a remote machine   
  * You can run a mp3 file using `nvlc`  
* Successfully copy a file to/from a remote machine
